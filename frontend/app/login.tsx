...
import { View, Text, StyleSheet, ImageBackground, Pressable, ScrollView, TextInput, Platform, Alert, KeyboardAvoidingView } from "react-native";
...
export default function Login() {
  ...
  const onAdminLogin = async () => {
    ...
    setBusy(true);
    try {
      const res = await api("/api/auth/admin/login", { method: "POST", body: JSON.stringify({ email: email.trim(), password }) });
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.top}>
            ...
            <View style={styles.card}>
              ...
              <Pressable testID="admin-login-btn" onPress={() => setShowAdmin((v) => !v)} style={styles.adminLink}>
                <Icon name={showAdmin ? "chevron-up" : "shield-crown"} size={16} color="#F5A623" />
                <Text style={styles.adminLinkTxt}>{showAdmin ? "إخفاء دخول الأدمن" : "دخول الأدمن"}</Text>
              </Pressable>

              {showAdmin && (
                <View style={styles.adminBox} testID="admin-box">
                  <Text style={styles.adminHint}>خاص بالمدير — استخدم بريد وكلمة مرور الأدمن</Text>
                  <TextInput
                  ...
                  <Pressable
                    ...
                    disabled={busy}
                    style={({ pressed }) => [styles.googleBtn, { marginTop: 6 }, pressed && { opacity: 0.85 }]}
                  >
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 20, overflow: "hidden", padding: 20, borderWidth: 1, borderColor: "rgba(245,166,35,0.35)", backgroundColor: "rgba(26,26,34,0.92)" },
  ...
  adminBox: { marginTop: 16, borderTopWidth: 1, borderTopColor: "#33333F", paddingTop: 14, gap: 10 },
  adminHint: { color: "#B0B0B8", fontSize: 11, textAlign: "center" },
  input: { backgroundColor: "#262630", color: "#F0F0F5", borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, borderWidth: 1, borderColor: "#33333F", textAlign: "right", fontSize: 14 },
});
